...
- [ ] Rearrange the secions. (see comment below)
- [ ] Add moar pix.
...
# Materials
Ask a manager if you would like to cut any materials not on either of these lists.
...
# General Notes
* This laser cutter will work with vector (usually DXF) files (for cutting and rastering) and image (usually JPG or PNG) files (rastering only).
...
1. You can change the cut type from cut (will just use the laser to etch the lines on your vector) to scan (will use the laser to ‘fill’ in your vector shape--usually used to etch opaque shapes)
    * Note: Scanning large images can take a very long time as opposed to cutting. If you can cut instead of scan, please do so. 
